...
export default function CadastroIngressos() {
  ...
  // Captura o ID da URL. Se a pasta for [id], o params.id terá o valor.
  ...
  // Sincroniza o ID vindo dos parâmetros da URL
  useEffect(() => {
    if (params?.id) {
      setId(params.id as string);
      console.log("ID do Evento identificado:", params.id);
    }
  }, [params]);
  ...
  const handleFinalizar = async () => {
    // 1. Validação de ID (Verifica se o ID existe e não é a string "undefined")
    if (!id || id === 'undefined' || id === '[id]') {
      Swal.fire({
        title: 'Erro de Identificação',
        text: 'ID do evento não identificado. Tente atualizar a página ou voltar ao passo anterior.',
        icon: 'error',
    ...
    try {
      // 3. Formatação para o Banco de Dados
