...
export default function CadastroIngressos({ params }: any) {
  ...
  // O ID agora vem direto das props da página
  const id = params?.id;
  ...
  const handleFinalizar = async () => {
    ...
    try {
      ...
      // TRATAMENTO: Convertemos preco e quantidade para números antes de enviar
      ...
      // Rota ajustada para bater com o seu Back-end: /api/eventos/ingressos/:id
      const response = await fetch(`${apiBaseUrl}/api/eventos/ingressos/${id}`, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        Swal.fire('Erro', 'O servidor salvou mas respondeu com erro. Verifique seus eventos.', 'error');
      }
    } catch (error) {
