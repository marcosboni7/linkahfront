...
export default function TabelaEventos() {
  ...
  const abrirModalEdicao = (evento: any) => {
    ...
    setPreviewUrl(evento.imagem_capa); // Mostra a imagem atual da AWS como preview inicial
  ...
  const handleSalvarEdicao = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // CRIAÇÃO DO FORMDATA PARA ENVIAR ARQUIVO
      ...
      formData.append('valor_minimo', eventoParaEditar.valor_minimo || '0');
      
      if (selectedFile) {
        formData.append('imagem', selectedFile); // O anexo vai aqui
      }
  ...
  return (
    <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden font-sans">
      ...
      {/* MODAL DE EDIÇÃO COMPLETO COM ANEXO */}
                ...
                >
                  {previewUrl ? (
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-[10px] font-bold text-slate-400 uppercase">Clique para selecionar arquivo</p>
                    </div>
                  )}
                </div>
                <input 
