            ...
            <div className="animate-in fade-in slide-in-from-bottom-6 duration-700">
              ...
              <div className="bg-slate-50 p-6 rounded-[2.5rem] mb-6 border border-slate-100">
                <p className="text-slate-600 font-bold text-sm leading-relaxed">
                  {userData.bio ? `"${userData.bio}"` : "Sem bio definida."}
                </p>
              </div>
              <div className="space-y-3">
                {userData.instagram && (
                  <a href={`https://instagram.com/${userData.instagram.replace('@','')}`} target="_blank" className="flex items-center justify-between p-4 bg-pink-50 rounded-2xl">
                    <div className="flex items-center gap-3"><Instagram size={18} className="text-pink-600"/><span className="text-pink-600 font-black text-[10px] uppercase italic">Instagram</span></div>
                    <ExternalLink size={14} className="text-pink-300"/>
                  </a>
                )}
              </div>
            </div>
          ) : <p className="text-center py-10 text-slate-400 font-bold uppercase text-xs">Perfil não encontrado</p>}
        </div>
      </div>
    </div>
  );
