...
export default function PerfilPage() {
  ...
  useEffect(() => {
    const carregarDados = async () => {
      ...
      // LOG DE TESTE: Verifique se isso aparece no seu F12
      console.log("📧 Buscando dados para o e-mail:", emailLogado);
      ...
      try {
        ...
        if (response.ok && data) {
          // O SEGREDO: Usar || '' para que valores NULL do banco virem campos vazios no site
          setFormData({
            nome: data.nome || '',
    ...
    <div className="min-h-screen bg-[#F1F5F9]">
      <nav className="bg-white px-8 py-4 flex justify-between items-center border-b border-slate-200 shadow-sm">
        ...
        <div className="flex items-center gap-3">
            <span className="text-slate-500 text-sm">{formData.nome}</span>
      ...
      <main className="max-w-[1000px] mx-auto p-6 md:p-10">
        <div className="bg-white rounded-3xl shadow-sm border border-slate-200 p-8 md:p-12">
          <h2 className="text-[#C22973] text-2xl font-bold mb-8 flex items-center gap-2">
            Dados Pessoais
          </h2>
          ...
          <form onSubmit={handleSalvar} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full border border-slate-200 rounded-xl p-4 outline-none focus:border-[#C22973] transition-all text-slate-700" 
                />
              </div>
              <div className="relative">
            ...
            <div className="flex justify-end pt-6">
              <button 
                type="submit" 
                disabled={isSaving} 
                className="bg-[#C22973] text-white px-12 py-4 rounded-2xl font-bold flex items-center gap-2 hover:bg-[#a62262] transform active:scale-95 transition-all disabled:opacity-50 shadow-lg shadow-pink-100"
              >
