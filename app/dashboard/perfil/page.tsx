...
function PerfilContent() {
  ...
  useEffect(() => {
    const carregarDados = async () => {
      ...
      if (!emailLogado) { router.push('/auth/login'); return; }
  ...
  const handleConectarStripe = async () => {
    ...
    try {
      ...
      Swal.fire('Erro', 'Falha ao conectar com Stripe.', 'error');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  if (isLoading) return (
    <div className="flex h-screen w-screen flex-col items-center justify-center bg-white gap-4">
      <div className="relative">
        ...
        <Zap className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[#FF4D4D]" size={16} />
      </div>
      <span className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 italic">Sincronizando Dados...</span>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#FDFDFF] p-6 md:p-12 font-sans">
      <div className="max-w-[850px] mx-auto">
        ...
        <div className="bg-white rounded-[4rem] shadow-2xl shadow-red-100/30 p-8 md:p-20 border border-slate-50 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-48 h-48 bg-gradient-to-br from-red-50/50 to-transparent rounded-bl-[8rem] -z-0"></div>
          ...
          <div className="flex items-center gap-8 mb-20 relative z-10">
            ...
            <div>
              ...
              <p className="text-slate-400 mt-3 font-bold uppercase text-[11px] tracking-[0.25em] italic flex items-center gap-2">
                <Zap size={14} className="text-[#FF4D4D]" /> Identidade do Produtor & Checkout
              </p>
            </div>
          </div>
          ...
          <form onSubmit={handleSalvar} className="space-y-16 relative z-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
              <div className="space-y-4">
                <label className={`text-[11px] font-black uppercase tracking-[0.2em] block ml-2 ${errors.nome ? 'text-red-500' : 'text-slate-400'}`}>Nome Completo / Social *</label>
                <input name="nome" value={formData.nome} onChange={handleChange} className="w-full border-2 border-slate-50 bg-slate-50/50 p-6 rounded-[2rem] outline-none focus:border-[#FF4D4D] focus:bg-white transition-all font-bold text-slate-800 shadow-sm text-sm" placeholder="Ex: João Silva Eventos" />
              </div>
              <div className="space-y-4">
                <label className="text-[11px] font-black uppercase tracking-[0.2em] block ml-2 text-slate-400">CPF ou CNPJ *</label>
                <input name="cpf_cnpj" value={formData.cpf_cnpj} onChange={handleChange} maxLength={18} className="w-full border-2 border-slate-50 bg-slate-50/50 p-6 rounded-[2rem] outline-none focus:border-[#FF4D4D] focus:bg-white transition-all font-bold text-slate-800 shadow-sm text-sm" placeholder="000.000.000-00" />
              </div>
            </div>

            <div className="pt-16 border-t border-slate-100">
              <div className="flex items-center gap-3 mb-10">
                <div className="w-10 h-10 bg-red-50 text-[#FF4D4D] rounded-xl flex items-center justify-center"><MapPin size={20} /></div>
                <h3 className="text-slate-900 font-black text-[11px] uppercase tracking-[0.3em] italic">Localização de Faturamento</h3>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                <div className="col-span-1 space-y-4">
                  <label className="text-[10px] text-slate-400 font-black uppercase tracking-widest ml-2">CEP</label>
                  <input name="cep" value={formData.cep} onChange={handleChange} onBlur={handleCepBlur} maxLength={9} className="w-full border-2 border-slate-50 bg-slate-50/50 p-6 rounded-[2rem] outline-none focus:border-[#FF4D4D] font-bold text-slate-800 text-center" />
                </div>
                <div className="col-span-2 space-y-4">
                  <label className="text-[10px] text-slate-400 font-black uppercase tracking-widest ml-2">Rua / Avenida</label>
                  <input name="rua" value={formData.rua} onChange={handleChange} className="w-full border-2 border-slate-50 bg-slate-50/50 p-6 rounded-[2rem] outline-none focus:border-[#FF4D4D] font-bold text-slate-800" />
                </div>
                <div className="col-span-1 space-y-4">
                  <label className="text-[10px] text-slate-400 font-black uppercase tracking-widest ml-2">Nº</label>
                  <input name="numero" value={formData.numero} onChange={handleChange} className="w-full border-2 border-slate-50 bg-slate-50/50 p-6 rounded-[2rem] outline-none focus:border-[#FF4D4D] font-bold text-slate-800 text-center" />
                </div>
              </div>
            </div>

            {/* STRIPE SECTION PREMIUM */}
            <div className="pt-16 border-t border-slate-100">
              <div className="flex items-center gap-3 mb-10">
                <div className="w-10 h-10 bg-emerald-50 text-emerald-500 rounded-xl flex items-center justify-center"><CreditCard size={20} /></div>
                <h3 className="text-slate-900 font-black text-[11px] uppercase tracking-[0.3em] italic">Gestão de Recebíveis (Stripe)</h3>
              </div>
              ...
              {stripeAtivo && stripeDetails ? (
                <div className="bg-emerald-50/30 border-2 border-emerald-100 p-10 rounded-[3rem] relative group hover:bg-emerald-50/50 transition-all">
                  <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                    <div className="flex items-center gap-6">
                      ...
                      <div>
                        <p className="text-emerald-950 font-black uppercase text-[12px] tracking-[0.1em]">{stripeDetails.business_name}</p>
                    ...
                    <div className="flex gap-3">
                       <span className="px-5 py-2 bg-white text-emerald-600 rounded-full text-[10px] font-black uppercase border border-emerald-100 shadow-sm tracking-widest flex items-center gap-2">
                        <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-ping" /> CONTA ATIVA
                       </span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-10">
                    <div className="bg-white/60 p-4 rounded-2xl border border-emerald-100/50 flex items-center justify-between">
                      <span className="text-[9px] font-black text-slate-500 uppercase italic">Vendas via Pix/Card</span>
                      <span className="text-[10px] font-black text-emerald-600 uppercase tracking-tighter">Habilitado</span>
                    </div>
                    <div className="bg-white/60 p-4 rounded-2xl border border-emerald-100/50 flex items-center justify-between">
                      <span className="text-[9px] font-black text-slate-500 uppercase italic">Saques Automáticos</span>
                      <span className="text-[10px] font-black text-emerald-600 uppercase tracking-tighter">Liberado</span>
                    </div>
                  </div>
                </div>
              ) : (
                <div className="bg-slate-50 p-10 rounded-[3rem] border-2 border-dashed border-slate-200 flex flex-col lg:flex-row items-center justify-between gap-10">
                  ...
                  <button type="button" onClick={handleConectarStripe} className="bg-slate-950 text-white px-10 py-5 rounded-[1.5rem] font-black text-[11px] uppercase tracking-[0.2em] flex items-center gap-3 hover:bg-[#FF4D4D] transition-all shadow-xl active:scale-95 group">
            ...
            <button type="submit" disabled={isSaving} className="w-full bg-[#FF4D4D] text-white py-8 rounded-[2.5rem] font-black uppercase tracking-[0.5em] italic flex items-center justify-center gap-5 hover:bg-slate-950 transition-all shadow-2xl shadow-red-200 hover:shadow-slate-300 disabled:opacity-50 active:scale-[0.98] group">
