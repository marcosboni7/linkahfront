...
import { 
  ...
  Trash2, Edit3, Save, X, Loader2, RefreshCcw, 
  LayoutDashboard, Ticket, MessageCircle, Lock, 
  UserMinus, UserCheck, Mail
} from 'lucide-react';
...
export default function AdminUsuarios() {
  ...
  const API_URL = 'https://linkah-api.onrender.com/api/usuarios'; // Ajuste conforme seu endpoint real
  ...
  // FUNÇÃO PARA BANIR / REATIVAR
  const handleToggleBan = async (user: any) => {
    const novoStatus = user.status === 'Banido' ? 'Ativo' : 'Banido';
    const confirmar = window.confirm(`Tem certeza que deseja mudar o status de ${user.nome} para ${novoStatus}?`);
  ...
  // SALVAR EDIÇÃO (NOME, SENHA, CARGO)
    ...
    <div className="flex min-h-screen bg-[#F4F5F7] text-slate-900 font-sans">
      ...
      <main className="flex-1 overflow-y-auto">
        <header className="bg-white border-b border-slate-200 px-10 py-8 flex items-center justify-between sticky top-0 z-10">
          <div>
            <h1 className="text-3xl font-black tracking-tight">Membros da Comunidade</h1>
            <p className="text-slate-400 text-sm font-medium">Gerencie acessos, altere senhas e modere usuários.</p>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50/50 text-slate-400 text-[10px] uppercase font-black tracking-widest border-b border-slate-100">
                  ...
                  <th className="px-8 py-6">Nível / Status</th>
                  ...
                  <tr key={user.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="px-8 py-6">
                      <div className="flex flex-col gap-1">
                        ...
                        <span className={`text-[10px] font-bold ${user.status === 'Banido' ? 'text-red-500' : 'text-emerald-500'}`}>
                          {user.status === 'Banido' ? 'CONTA BLOQUEADA' : 'CONTA ATIVA'}
                        </span>
                      </div>
                    </td>
                      ...
                      <div className="flex justify-end gap-3">
                        ...
                        <button 
                          ...
                          className={`p-3 border rounded-xl transition-all shadow-sm ${
                            ...
                            : 'bg-white text-slate-400 border-slate-200 hover:bg-red-500 hover:text-white'
                          }`}
                          title={user.status === 'Banido' ? "Desbanir" : "Banir Usuário"}
                        >
          ...
          <div className="bg-white w-full max-w-lg rounded-[3.5rem] shadow-2xl overflow-hidden animate-in zoom-in duration-300">
            ...
            <form onSubmit={salvarEdicao} className="p-10 space-y-6">
              <div className="space-y-2">
                <label className="text-[11px] font-black uppercase text-slate-400 tracking-widest">Nome Completo</label>
                <input 
              ...
              <div className="space-y-2">
                <label className="text-[11px] font-black uppercase text-[#ff4d4d] tracking-widest">Nova Senha (Mudar apenas se necessário)</label>
                <input 
                  type="password"
                  placeholder="Digite a nova senha..."
                  className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-5 font-bold outline-none focus:ring-4 ring-[#ff4d4d]/10 transition-all"
                  onChange={(e) => setUserParaEditar({...userParaEditar, senha: e.target.value})}
                />
                <p className="text-[10px] text-slate-400 font-medium italic">Deixe em branco para manter a senha atual.</p>
              </div>

              <div className="space-y-2">
                <label className="text-[11px] font-black uppercase text-slate-400 tracking-widest">Nível de Acesso</label>
                <select 
                  className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-5 font-bold outline-none"
                  value={userParaEditar.role}
              ...
              <button 
                type="submit" 
                disabled={isProcessing === userParaEditar.id}
