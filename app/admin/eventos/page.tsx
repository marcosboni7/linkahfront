...
export default function AdminEventos() {
  ...
  const [eventoParaEditar, setEventoParaEditar] = useState<any>({
    id: '', nome: '', data: '', horario: '', local: '', preco: '', imagem: '', descricao: '', categoria: '', status: ''
  });
  ...
  const abrirEdicao = (evento: any) => {
    setEventoParaEditar({
      ...evento,
      data: evento.data || evento.data_inicio || '',
  ...
  const salvarEdicao = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      data_inicio: eventoParaEditar.data, // Salvando nos dois campos para garantir
  ...
  return (
    <div className="p-10 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
      <header className="flex justify-between items-center">
        ...
        <button onClick={carregarDados} className="p-4 bg-white border border-slate-200 rounded-2xl hover:bg-slate-50">
            ...
            {eventos.map((ev) => (
              <tr key={ev.id} className="group hover:bg-slate-50/50">
                ...
                <td className="px-8 py-6 text-xs font-bold text-slate-600">
                  <span className="flex items-center gap-1"><Calendar size={12}/> {ev.data || ev.data_inicio}</span>
                  <span className="flex items-center gap-1"><Clock size={12}/> {ev.horario}</span>
                </td>
                <td className="px-8 py-6 text-right">
                  <button onClick={() => abrirEdicao(ev)} className="p-3 hover:text-blue-600 transition-all"><Edit3 size={18}/></button>
                </td>
              </tr>
            ))}
          </tbody>
          ...
          <div className="bg-white w-full max-w-xl rounded-[3rem] p-10 shadow-2xl overflow-y-auto max-h-[90vh]">
            ...
            <form onSubmit={salvarEdicao} className="grid grid-cols-2 gap-5">
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase text-slate-400 ml-2">Data (AAAA-MM-DD)</label>
                <input type="text" className="w-full p-4 bg-slate-50 rounded-2xl font-bold outline-none border border-transparent focus:border-slate-200" 
                  value={eventoParaEditar.data} onChange={(e) => setEventoParaEditar({...eventoParaEditar, data: e.target.value})} />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase text-slate-400 ml-2">Horário</label>
                <input className="w-full p-4 bg-slate-50 rounded-2xl font-bold outline-none border border-transparent focus:border-slate-200" 
                  value={eventoParaEditar.horario} onChange={(e) => setEventoParaEditar({...eventoParaEditar, horario: e.target.value})} />
              </div>
              <button type="submit" className="col-span-2 bg-slate-900 text-white py-5 rounded-3xl font-black uppercase tracking-widest hover:bg-red-500 transition-all">
                {isProcessing === 'salvando' ? 'Salvando...' : 'Confirmar e Salvar'}
              </button>
            </form>
          </div>
        </div>
