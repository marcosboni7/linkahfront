...
export default function SalaLinkahSkype() {
  ...
  const handleImageError = (e: any, local: string) => e.target.src = DEFAULT_FOTO;

  const iniciarCall = async (destino: string) => {
    if (!dadosUsuario) return;
    const sala = `Call_${id}_${Date.now()}`;
    const fotoCall = dadosUsuario.foto_perfil || dadosUsuario.avatar || dadosUsuario.usuario_foto || dadosUsuario.foto || null;
    try {
  ...
  const enviarMensagem = async (e: any) => {
    ...
    const fotoUsuario = dadosUsuario.foto_perfil || dadosUsuario.avatar || dadosUsuario.usuario_foto || dadosUsuario.foto || null;

    const payload = {
      evento_id: Number(id),
      usuario_nome: dadosUsuario.nome,
      usuario_foto: fotoUsuario,
      texto: novoTexto,
  ...
  useEffect(() => {
    ...
    const atualizar = async () => {
      try {
        const minhaFoto = dadosUsuario.foto_perfil || dadosUsuario.avatar || dadosUsuario.usuario_foto || dadosUsuario.foto || '';
          ...
          msgs.slice(-5).forEach((msg: any) => {
            if (msg.texto?.includes("CALL_INVITE|")) {
              ...
              if (destino === dadosUsuario.nome.toLowerCase() && (AGORA - new Date(msg.criado_em).getTime()) / 1000 < 25 && !chamadaAtiva) {
                setConviteRecebido({ de: msg.usuario_nome, sala: partes[2], foto: msg.usuario_foto || msg.foto || msg.avatar });
              }
            }
          });
        }
