...
import { 
  ...
  Loader2, Plus, Minus, ChevronLeft, Heart, Users 
} from 'lucide-react';
...
export default function DetalhesEvento() {
  ...
  useEffect(() => {
    async function carregarEvento() {
      ...
      try {
        setLoading(true);
        // O timestamp ?t= evita que o navegador mostre dados antigos após um UPDATE
  ...
  if (!evento) return (
    <div className="p-20 text-center">
      <p className="text-slate-500 font-medium mb-4">Evento não encontrado.</p>
      <Link href="/" className="text-[#E30031] font-bold underline">Voltar para a página inicial</Link>
    </div>
  );

  const listaIngressos = evento.ingressos || [];
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-24">
        ...
        <div className="flex justify-between items-center mb-6">
          <Link href="/" className="inline-flex items-center gap-2 text-slate-500 hover:text-slate-800 transition-colors text-sm font-semibold">
            <ChevronLeft size={18} /> Explorar Eventos
          </Link>
          <div className="flex gap-2">
            <button className="p-2.5 rounded-full border border-slate-200 hover:bg-slate-50 text-slate-600"><Share2 size={18} /></button>
            <button className="p-2.5 rounded-full border border-slate-200 hover:bg-slate-50 text-slate-600"><Heart size={18} /></button>
          </div>
        </div>

        {/* CAPA */}
        <div className="relative w-full aspect-[21/9] rounded-[2.5rem] overflow-hidden shadow-2xl mb-12 bg-slate-100">
          <img 
          ...
          <div className="absolute bottom-8 left-8 md:bottom-12 md:left-12 text-white">
             ...
             <h1 className="text-3xl md:text-5xl lg:text-6xl font-black tracking-tighter leading-none">{evento.nome}</h1>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="flex items-start gap-4 p-6 rounded-3xl bg-slate-50 border border-slate-100">
                ...
                <div>
                  ...
                  <p className="font-bold text-slate-800">
                    {/* TRATAMENTO DE DATA ISO PARA LOCAL SEM FUSO HORÁRIO */}
                    {evento.data_inicio ? new Date(evento.data_inicio + 'T12:00:00').toLocaleDateString('pt-BR') : 'A definir'}
                  </p>
              ...
              <div className="flex items-start gap-4 p-6 rounded-3xl bg-slate-50 border border-slate-100">
                ...
                <div>
                  ...
                  <p className="font-bold text-slate-800 line-clamp-1">{evento.tipo === 'Online' ? 'Online' : (evento.local_nome || 'Local')}</p>
              ...
              <div className="flex items-start gap-4 p-6 rounded-3xl bg-slate-50 border border-slate-100">
                ...
                <div>
                  ...
                  <p className="font-bold text-slate-800 line-clamp-1">{evento.produtor_nome || 'Linkah Produtora'}</p>
              ...
              <div className="bg-white rounded-[3rem] border-2 border-slate-100 shadow-xl overflow-hidden">
                <div className="p-8 md:p-10 space-y-8">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="bg-green-100 text-green-600 text-[10px] font-black px-2 py-1 rounded">ATIVO</span>
                  </div>
                  ...
                  <div className="space-y-4">
                    {listaIngressos.length > 0 ? (
                      listaIngressos.map((ing: any, idx: number) => (
                        <div key={idx} className="flex items-center justify-between border-b border-slate-50 pb-4">
                          <div>
                            ...
                            <p className="text-xs text-slate-400">Restam {ing.quantidade}</p>
                          </div>
                      ...
                      <p className="text-slate-400 text-sm italic text-center">Valores sob consulta</p>
                    )}

                    <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl">
                      <p className="text-[10px] font-black text-slate-400 uppercase">Qtd</p>
                  ...
                  <div className="pt-6 border-t-2 border-dashed border-slate-100 space-y-4">
                    ...
                    <Link 
                      ...
                      className="flex items-center justify-center w-full bg-[#E30031] py-6 rounded-[2rem] font-black text-white uppercase text-xs tracking-widest gap-3 hover:bg-red-700 transition-colors"
                    >
                      <Ticket size={20} /> Comprar Agora
                    </Link>
                  </div>
                </div>
              </div>
