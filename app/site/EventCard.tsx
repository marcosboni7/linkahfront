...
const EventCard = dynamic(() => import('../site/EventCard'), {
  ssr: false,
        ...
        {!loading && comunidades.length > 0 && (
          <section className="space-y-10">
            <div className="flex items-end justify-between border-b border-slate-100 pb-6">
              ...
              <Link
                href="/comunidades"
                className="text-slate-500 font-bold text-xs uppercase tracking-widest flex items-center gap-2 hover:text-slate-900 transition-colors group"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {comunidades.map((com) => {
                ...
                if (rawFoto && typeof rawFoto === 'string') {
                  const valor = rawFoto.trim();

                  if (valor.startsWith('http://') || valor.startsWith('https://')) {
                    fotoFinal = valor;
                  } else if (valor.startsWith('linkah/eventos/')) {
                    fotoFinal = `https://res.cloudinary.com/${CLOUDINARY_CLOUD_NAME}/image/upload/${valor}`;
                  } else {
                    fotoFinal = `${API_URL_BASE}/uploads/${valor.replace(/^\/+/, '')}`;
                  }
                }
                ...
                return (
                  <Link
                    ...
                    className="group bg-white rounded-3xl border border-slate-100 hover:border-slate-200 hover:shadow-xl transition-all duration-300 flex flex-col overflow-hidden"
                  >
                    <div className="relative h-48 w-full overflow-hidden bg-slate-50">
                      <Image
                        src={fotoFinal}
                        alt={com.nome || "Comunidade"}
                        fill
                        unoptimized
                    ...
                    <div className="p-6 flex-grow">
                      ...
                      <h3 className="text-xl font-black text-slate-900 mb-2 group-hover:text-[#ff4d4d] transition-colors uppercase tracking-tight">
                        {com.nome || "Sem Nome"}
                      </h3>
